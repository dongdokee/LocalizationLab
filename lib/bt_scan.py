...
class ScanResult:
    ...
    def __init__(self, t: float, scan_result_entries: Optional[List[ScanResultEntry]]=None):
        self.t = t 
    ...
    def __repr__(self) -> str:
        return f"ScanResult(t: {self.t}, scan_result_entries: {self.scan_result_entries})"
...
async def scan(report_interval, processed_callback, *args, **kwargs):
    async def callback(device, advertisement_data):
        ...
        if hasattr(advertisement_data, 'service_data') and advertisement_data.service_data:
            for service_uuid, service_payload in advertisement_data.service_data.items():
    ...
    async with BleakScanner(callback) as scanner:
        process_task = asyncio.create_task(process(report_interval, processed_callback, *args, **kwargs))
...
async def process(report_interval, processed_callback, *args, **kwargs):
    ...
    while True:
        ...
        if data[0] - bin_start_time > report_interval:
            # ScanResult 생성
            scan_result_entries = [ScanResultEntry(uuid, rssi) for _, uuid, rssi in process_queue]
